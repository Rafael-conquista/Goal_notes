		...
		<div className='consulta_skins'>
			...
			<div className='amizades_amigos consulta_skins_images'>
				{skin.length > 0 ? skin.map((skinsUsuario) => (
					<>
						{skinsUsuario.type == 2 &&
                          <span onClick={() => escolherFundo(skinsUsuario.id, skinsUsuario.type)} className="espacamento text_store"><div className={`img_perfil img_store background_${skinsUsuario.enum}`}></div></span>
						}
					</>
			...
			<div className='amizades_amigos consulta_skins_images'>
            	...
            	<span onClick={() => escolherFundoPadrao(2)} className="espacamento text_store"><div className={`img_perfil img_store background_0`}></div></span>
			</div>
		</div>
	  </div>
	);
